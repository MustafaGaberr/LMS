...
import { submitPreSurvey } from '../api/surveyApi';
import './Survey.css';
...
const PreSurvey: React.FC = () => {
    const navigate = useNavigate();
    const { scaleId } = useParams<{ scaleId: string }>();
    const submitPreScaleResponse = useAppStore((s) => s.submitPreScaleResponse);
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            await submitPreSurvey(activeScale.scaleType, answersArray);
            submitPreScaleResponse(activeScale.scaleType, responses);
            setPhase('done');
            window.scrollTo({ top: 0, behavior: 'smooth' });
        } catch {
    ...
    }, [allAnswered, isSubmitting, activeScale, responses, submitPreScaleResponse]);
    ...
    if (!activeScale) {
        return (
            <div className="survey-page survey-page--locked">
                ...
                <Button variant="primary" onClick={() => navigate('/pre-scales')}>
                    العودة للمقاييس
                </Button>
            </div>
        );
    ...
    const alreadyFilled =
        (activeScale.scaleType === 'cognitive' && progress.preCognitiveFilled) ||
        (activeScale.scaleType === 'efficacy' && progress.preEfficacyFilled);

    if (alreadyFilled) {
        const savedResponses = progress.surveyResponses ?? {};
        // Pre responses are stored with "pre_" prefix
        const preResponses: Record<string, number> = {};
        for (const [k, v] of Object.entries(savedResponses)) {
            if (k.startsWith('pre_')) {
                preResponses[k.replace('pre_', '')] = v;
            }
        }
        let globalQ = 0;
        return (
                            ...
                            {section.questions.map((q) => {
                                ...
                                return (
                                    <QuestionCard
                                        ...
                                        question={q}
                                        index={globalQ}
                                        selectedValue={preResponses[q.id]}
                                        onSelect={() => {}}
                                        readonly
                                    />
                                );
    ...
    return (
        <div className="survey-page">
            ...
            <div key={animKey} className={`survey-section-anim survey-section-anim--${animDir}`}>
                {currentSection && (
                    <SectionHeader
